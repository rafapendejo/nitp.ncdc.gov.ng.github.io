...
{if(zoom)
$gmap.setZoom(zoom);$gmap.panTo(new google.maps.LatLng(parseFloat(latitude),parseFloat(longitude)));});var overlays=[];$(this).bind('gMap.clearMarkers',function()
...
gmarker.setMap($gmap);overlays.push(gmarker);});for(var j=0;j<opts.markers.length;j++)
